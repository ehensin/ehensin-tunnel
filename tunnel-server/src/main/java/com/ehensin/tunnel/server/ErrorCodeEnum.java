...
public enum ErrorCodeEnum {
	...
	TunnelChannelHttpReponseError("100007","http server response error status"),
	/*codec error*/		
	CodecEncodeError("200001","cannot encode message to target type"),
	CodecDecodeError("200002","cannot decode message to target type"),
	/*rpc error*/
	RpcInvokerLocationError("300001","cannot find service"),
	RpcInvokeError("300002","cannot invoke service or cannot parse invoke result"),
	/*plain text call error*/
	PlaintextInvokerLocationError("400001","cannot find plain text service"),
	PlaintextInvokeError("400002","cannot invoke plain text service or cannot parse invoke result")
    ;
